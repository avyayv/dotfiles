...
function _vsp_agent_command() {
  case "$1" in
    codex|claude) printf '%s --yolo' "$1" ;;
    pi) printf 'pi' ;;
  esac
}

function _vsp_send_prompt() {
  local target="$1"
  local prompt="$2"
  [ -z "$prompt" ] && return 0

  # Give the agent TUI a moment to finish starting before pressing Enter.
  (
    sleep 2
...
function _vsp_copy_worktree_context() {
  local source_dir="$1"
  local target_dir="$2"
  local patch_file="$3"

  if [ -s "$patch_file" ]; then
    (cd "$target_dir" && git apply --3way "$patch_file") || return 1
  fi

  # Copy untracked, non-ignored files so both agents start from the same local context.
  (cd "$source_dir" && git ls-files --others --exclude-standard -z) | \
    (cd "$source_dir" && tar --null -T - -cf - 2>/dev/null) | \
    (cd "$target_dir" && tar -xf - 2>/dev/null)

  # Match gwt behavior: share local env files and node_modules into the temp worktrees.
  while IFS= read -r env_file; do
    local rel_path="${env_file#$source_dir/}"
    local target_path="$target_dir/$rel_path"
    [ -e "$target_path" ] && continue
    mkdir -p "$(dirname "$target_path")"
    ln -s "$env_file" "$target_path"
  done < <(find "$source_dir" -name ".env*" -type f 2>/dev/null)
  ...
  while IFS= read -r nm_dir; do
    ...
    [ -e "$target_path" ] && continue
    mkdir -p "$(dirname "$target_path")"
    ln -s "$nm_dir" "$target_path"
  done < <(find "$source_dir" -name "node_modules" -type d -prune 2>/dev/null)

  return 0
}

# Create an iTerm/tmux vertical split with two coding agents, each in its own
# fresh worktree cloned from the current branch/worktree. The current/base
# worktree opens pi as the judge/coordinator.
# Usage: vsp [codex|pi|claude] [codex|pi|claude] [session_name] [-- prompt]
# Examples:
#   vsp                                      # codex + pi
#   vsp codex claude
#   vsp pi claude my-ab -- "implement X"
function vsp() {
  local -a positional=()
  local prompt=""
  while [ "$#" -gt 0 ]; do
  ...
  local left="${positional[1]:-codex}"
  local right="${positional[2]:-pi}"
  local requested_session="${positional[3]:-}"
  local source_dir=""
  local repo_name=""
  local current_ref=""
  local safe_ref=""
  local stamp="$(date +%Y%m%d-%H%M%S)-$$"
  local wt_base="$HOME/.avyay-worktrees"
  local left_wt=""
  local right_wt=""
  local left_branch=""
  local right_branch=""
  ...
  if [ "$left" = "--help" ] || [ "$left" = "-h" ]; then
    echo "Usage: vsp [codex|pi|claude] [codex|pi|claude] [session_name] [-- prompt]"
    echo "Examples: vsp  |  vsp codex claude  |  vsp pi claude my-ab -- 'implement X'"
    return 0
  fi
  ...
  case "$left" in
    codex|pi|claude) ;;
    *) echo "Unknown agent: $left (expected codex, pi, or claude)"; return 1 ;;
  esac

  case "$right" in
    codex|pi|claude) ;;
    *) echo "Unknown agent: $right (expected codex, pi, or claude)"; return 1 ;;
  esac

  if [ "$left" = "$right" ]; then
    echo "Pick two different agents"
    return 1
  fi
  ...
  if ! command -v "$left" >/dev/null 2>&1; then
    echo "Missing command: $left"
    return 1
  fi

  if ! command -v "$right" >/dev/null 2>&1; then
    echo "Missing command: $right"
    return 1
  fi
  ...
  left_cmd="$(_vsp_agent_command "$left")"
  right_cmd="$(_vsp_agent_command "$right")"

  source_dir="$(git rev-parse --show-toplevel 2>/dev/null)" || {
    echo "vsp must be run inside a git repository"
    return 1
  }

  repo_name="$(basename "$source_dir")"
  current_ref="$(git -C "$source_dir" rev-parse --abbrev-ref HEAD 2>/dev/null)"
  if [ "$current_ref" = "HEAD" ]; then
    current_ref="$(git -C "$source_dir" rev-parse --short HEAD 2>/dev/null)"
  fi
  safe_ref="$(printf '%s' "$current_ref" | tr -c '[:alnum:]_.-' '-')"

  mkdir -p "$wt_base"
  left_wt="$wt_base/${repo_name}-${safe_ref}-vsp-${left}-${stamp}"
  right_wt="$wt_base/${repo_name}-${safe_ref}-vsp-${right}-${stamp}"
  left_branch="vsp/${safe_ref}/${left}-${stamp}"
  right_branch="vsp/${safe_ref}/${right}-${stamp}"

  tmp_dir="$(mktemp -d)" || return 1
  patch_file="$tmp_dir/tracked.patch"
  git -C "$source_dir" diff --binary HEAD > "$patch_file"

  echo "Creating worktrees:"
  echo "  $left  -> $left_wt"
  echo "  $right -> $right_wt"

  git -C "$source_dir" worktree add -b "$left_branch" "$left_wt" HEAD || { rm -rf "$tmp_dir"; return 1; }
  git -C "$source_dir" worktree add -b "$right_branch" "$right_wt" HEAD || {
    git -C "$source_dir" worktree remove --force "$left_wt" >/dev/null 2>&1
    rm -rf "$tmp_dir"
    return 1
  }

  _vsp_copy_worktree_context "$source_dir" "$left_wt" "$patch_file" || echo "Warning: could not copy all local context into $left_wt"
  _vsp_copy_worktree_context "$source_dir" "$right_wt" "$patch_file" || echo "Warning: could not copy all local context into $right_wt"
  rm -rf "$tmp_dir"

  if [ -n "$prompt" ]; then
    judge_prompt="$(cat <<EOF
You are the judge/coordinator for an A/B coding-agent run.
...
Let both contestants work independently. When they finish, inspect their diffs, run relevant checks if possible, decide which did better, apply or cherry-pick the winning work into this base worktree only if asked, and clean up the temporary worktrees/branches when safe.
EOF
)"
  fi
  ...
  if [ -n "$TMUX" ]; then
    ...
    left_pane="$(tmux new-window -P -F '#{pane_id}' -t "$window_target" -n "${left}-${right}" -c "$left_wt" "zsh -lic '$left_cmd; exec zsh -l'")"
    window_index="$(tmux display-message -p -t "$left_pane" '#I')"
    right_pane="$(tmux split-window -P -F '#{pane_id}' -h -t "$session_name:$window_index" -c "$right_wt" "zsh -lic '$right_cmd; exec zsh -l'")"
    tmux select-layout -t "$session_name:$window_index" even-horizontal
    _vsp_send_prompt "$left_pane" "$prompt"
    _vsp_send_prompt "$right_pane" "$prompt"
    _vsp_send_prompt "$current_pane" "$judge_prompt"
    echo "Opened $left_cmd and $right_cmd in a vertical split. Starting pi judge here."
    exec pi
  fi

  base_session="${requested_session:-vsp-${left}-${right}}"
  ...
  tmux new-session -d -s "$session_name" -n "judge" -c "$source_dir" "zsh -lic 'pi; exec zsh -l'"
  judge_pane="$(tmux list-panes -t "$session_name:" -F '#{pane_id}' | head -1)"
  judge_window_index="$(tmux display-message -p -t "$judge_pane" '#I')"
  left_pane="$(tmux new-window -P -F '#{pane_id}' -t "$session_name:" -n "${left}-${right}" -c "$left_wt" "zsh -lic '$left_cmd; exec zsh -l'")"
  window_index="$(tmux display-message -p -t "$left_pane" '#I')"
  right_pane="$(tmux split-window -P -F '#{pane_id}' -h -t "$session_name:$window_index" -c "$right_wt" "zsh -lic '$right_cmd; exec zsh -l'")"
  tmux select-layout -t "$session_name:$window_index" even-horizontal
  _vsp_send_prompt "$left_pane" "$prompt"
  _vsp_send_prompt "$right_pane" "$prompt"
  _vsp_send_prompt "$judge_pane" "$judge_prompt"
  tmux select-window -t "$session_name:$judge_window_index"

  # Attach with iTerm2 tmux integration so the judge opens in this iTerm tab and
  # the contestant split opens as a second tmux/iTerm tab.
  tmux -CC attach -t "$session_name"
}

# Backwards-compatible name for the A/B split helper.
function agentab() { vsp "$@"; }
